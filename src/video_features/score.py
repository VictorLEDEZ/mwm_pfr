...
def score(frame_list, shots, sampling_rate=10):
    ...
    for video in frame_list :
        ...
        sift_video = Sift(video[::sampling_rate], frame_shift=1, display = False, save_video = False, sampling_rate=sampling_rate) # PATENTED ?
        flow_video = Flow(video[::sampling_rate], frame_shift=1, display = False, save_video = False, sampling_rate=sampling_rate)
    ...
    dict_shots_order, df_order = shots_order(shots, agreg_agreg, len_cum, display=True)
